...
}>({ videoId: null, previewTime: null });
const previewYTPlayerAtom = atomWithReset<YT.Player | null>(null);

export const usePreviewVideoInfo = () =>
  useAtomValue(previewVideoInfoAtom, { store });
export const getPreviewVideoInfo = () => store.get(previewVideoInfoAtom);
export const setPreviewVideoInfo = (
  info: ExtractAtomValue<typeof previewVideoInfoAtom>,
) => {
...
export const usePreviewYTPlayer = () =>
  useAtomValue(previewYTPlayerAtom, { store });
export const getPreviewYTPlayer = () => store.get(previewYTPlayerAtom);
export const setPreviewYTPlayer = (newYTPlayer: YT.Player) =>
  store.set(previewYTPlayerAtom, newYTPlayer);
