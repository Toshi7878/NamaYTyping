...
(function () {
  ...
  const pageWindow = new Function("return unsafeWindow")();
...
jsxRuntimeExports.jsx(
  ...
  const useLiveChatSession = (inputRef, onConnect, onChat2, onError, onEnd2) => {
    ...
    reactExports.useEffect(() => {
      ...
      const ime = pageWindow.__ytyping_ime;
      if (ime) {
      ...
      return () => {
        ...
        const imeOnCleanup = pageWindow.__ytyping_ime;
        if (imeOnCleanup) {
  ...
  const NamaTypingContainer = () => {
    ...
    return jsxRuntimeExports.jsx(
      ImeLiveChatConnector,
      {
        ...
        onConnect: () => pageWindow.__ytyping?.toast.success("ライブチャットに接続しました"),
        onEnd: () => onEnd(chatStatesRef.current),
        onError: (e) => pageWindow.__ytyping?.toast.error(`接続エラー: ${e.message}`)
      }
    );
  };
  function onChat(messages, chatStates) {
    for (const m of messages) {
      ...
      const result = pageWindow.__ytyping_ime?.evaluateImeInput({
        value: m.message,
      ...
      pageWindow.__ytyping_ime?.addNotifications(
  ...
  function onEnd(chatStates) {
    chatStates.forEach(({ author, typeCount }) => {
      pageWindow.__ytyping_ime?.addUserResult({
        name: author,
        typeCount
      });
    });
  ...
  function getChatState(author, chatStatesRef) {
    if (!chatStatesRef.has(author)) {
      const initWordResults = pageWindow.__ytyping_ime?.getBuiltMap()?.initWordResults ?? [];
...
jsxRuntimeExports.jsx(
  ...
  const getInitialMode = () => {
    try {
      const stored = pageWindow.sessionStorage.getItem("mapLinkMode");
...
jsxRuntimeExports.jsx(
              Switch,
              {
                ...
                onCheckedChange: (checked) => {
                  ...
                  setMode(newMode);
                  pageWindow.__ytyping?.setMapLinkMode?.(newMode);
                }
              }
            )
          ]
